...
export default function PulseWizardPage() {
  ...
  const calculateCost = () => {
    let baseCost = formData.testing.quantity * 30; // $60 per tester

    // Additional costs based on methods
    if (formData.testing.methods.aiAssistedCalls) baseCost += formData.testing.quantity * 1;
    if (formData.testing.methods.interviews) baseCost += formData.testing.quantity * 25;
    if (formData.testing.methods.focusgroups) baseCost += formData.testing.quantity * 15;
